...
public enum ExceptionMessage {
    ...
    HAS_BOTH_ENDS_SEPARATOR(
            "입력값 시작 또는 끝에 구분자가 포함되어 있습니다. 다시 입력해 주세요."),
    INVALID_NUMBER_ERROR("숫자가 아닙니다. 다시 입력해 주세요."),
    LOTTO_NUMBER_OUT_OF_RANGE(
            format("%d~%d 사이의 숫자를 입력해 주세요.",
    ...
    WINNING_NUMBERS_AND_BONUS_NUMBER(
            "로또 번호와 보너스 번호 사이에 중복이 있습니다. 다시 입력해 주세요."),
    WINNING_NUMBERS_ERROR("1~45 사이의 숫자 6개를 공백 없이 %s로 구분하여 입력해 주세요."),
    WINNING_NUMBERS_SIZE_ERROR(
            format("%d개의 숫자를 입력해 주세요.", VALIDATE_LOTTO_SIZE));
